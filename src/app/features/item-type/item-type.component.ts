...
export class ItemTypeComponent implements OnInit, OnDestroy {
  ...
  // Table Options
  displayedColumns = ['itemName','action'];
  ...
  ngOnInit() {
    ...
    this.latestSearchFilter
        .debounceTime(300)
        .switchMap( search => this._itemTypeService.getDataSource(this.paginator,this.sort,search) )
        .subscribe( response => {
  ...
  initData(){
    ...
    this._loader.openSpinner(); // Dialog Progress Spinner
     ...
     this._itemTypeService.getDataSource(initPaginator,this.sort,this.searchFilter)
        .subscribe( response => {
            this._tableDatabaseService.tableDataStream$.next(response.data);
